...
export let sp_key: string = "1";  //translation results
...
export default class SugCompletionItemProvider implements vscode.CompletionItemProvider {
	public provideCompletionItems(document: vscode.TextDocument, position: vscode.Position, token: vscode.CancellationToken): Thenable<vscode.CompletionItem[]> {
		...
		let src_words = this.getInputString(activeLineText);
		if (src_words !== '' && src_words !== undefined) {
			return this.translation(src_words,document.languageId);
		}
	}
	 ...
	 * @param src_words
	 */
	public translation(src_words: string,language:string): Thenable<vscode.CompletionItem[]> {
				...
				.then(function (res) {
					...
					if (info.basic !== undefined
						...
						for (i = 0; i < 3; i++) {
							...
							completionItem.filterText = sug[3]+'1';
							completionItem.insertText = (language=='php'?'$':'') + sug[i]
